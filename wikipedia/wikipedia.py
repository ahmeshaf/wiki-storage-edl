...
def search(query, results=10, suggestion=False):
  ...
  * suggestion - if True, return results and suggestion (if any) in a tuple
  ...
  search_results = ((d['pageid'],d['title']) for d in raw_results['query']['search'])
...
def entity_content(title):
    p = page(title)
    return p.content

def disambiguation_links(tite):
    try:
        page(tite)
...
def backlinks(title, limit=10):
  '''
  This is to get the backlinks of a title
  :param title:
  :param limit:
  :return:
  '''
  ...
  backlinks = _wiki_request(params)["query"]["backlinks"]
  return [dict['title'] for dict in backlinks]

def outlinks(title, limit=10):
    """
    Allows you to query a limited amount of outlinks.
    :param title:
    :param limit:
    :return:
    """
    ...
    pages = _wiki_request(params)["query"]["pages"]
    key = list(pages.keys())[0]
    outlinks = pages[key]['links']
    return [dict['title'] for dict in outlinks]

def category_members(categoryname, limit=10):
  params = {
    "action": "query",
    "list": "categorymembers",
    "cmtitle": "Category:{}".format(categoryname),
    "cmlimit": str(limit),
  }
  category_members = _wiki_request(params)["query"]["categorymembers"]
  return [dict['title'] for dict in category_members]

def page_views(title, start="2016010100", end="2016123100"):
  '''
  get total page views of a title from say: Jan 1 2016 to Dec 31 2016
  :param title:
  :param start:
  :param end:
  :return:
  '''
  underscored_title = "_".join([word.strip() for word in title.split(' ')])
  rest_url = "https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia/all-access/user/{}/monthly/{}/{}".format(underscored_title, start, end)
  json = requests.get(rest_url).json()
  return sum([js["views"] for js in json['items']])

def mention_context(title, mention):
    '''
    return all the para matches of a mention within the content of a title
    para needs to have greater than or equal to 10 words
    :param title:
    :param mention:
    :return: [(para, span)]
    '''
    text = entity_content(title)
    paras = [para.strip() for para in text.split('\n') if para.strip() != '']
    matches = []
    for para in paras:
        m = re.search(mention.strip(), para)
        if m != None and len(para.split(' ')) >= 10:
            matches.append((para, m.span()))
    return matches
  ...
  * preload - load content, summary, images, references, and links during initialization
  ...
  if title is not None:
    if auto_suggest:
      ...
      try:
        title = suggestion or results[0][1]
      except IndexError:
