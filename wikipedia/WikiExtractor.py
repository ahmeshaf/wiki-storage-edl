#!/media/rehan/yaws/sem3/ml/wiki/bin/python
# -*- coding: utf-8 -*-
...
class Extractor(object):
    ...
    def extract(self, out):
        ...
        lines = text.split('\n')
        wikilines = [self.wiki2text(line) for line in lines]
...
def makeInternalLink(title, label):
    ...
    if options.keepLinks:
        replaced = '<a title="{}">{}</a>'.format(title, label).encode('utf-8')
        return str(replaced, 'utf-8')
    else:
        return label
...
def process_dump(text, template_file, out_file, file_size, file_compress,
                 process_count):
    """
...
def main(text):
    ...
    process_dump(text, args.templates, output_path, file_size,
